...
export default function Loader() {
  // Lazy initial state — read sessionStorage once before first render.
  // Avoids both the lint warning AND a flash of the loader on revisits.
  const [done, setDone] = useState<boolean>(() => {
    if (typeof window === "undefined") return false;
    return sessionStorage.getItem("fc-loaded") === "1";
  });
      ...
      {!done && (
        ...
        >
          <div className="relative">
            ...
            >
              Frame{" "}
              <span className="italic text-[#c89b5a]">&amp;</span> Code
            </motion.h1>
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
